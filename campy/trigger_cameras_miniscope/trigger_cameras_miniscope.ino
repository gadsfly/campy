...
// hard-coded pin 10 for controlling miniscope DC step pulse
int DIG_out_miniscope = 10;
...
int SetDigPins() {
  ...
  // Control the miniscope Pin and state
  
 
  pinMode(DIG_out_miniscope, OUTPUT);
  digitalWrite(DIG_out_miniscope, LOW);
...
void loop( void ) {
  ...
  if (frame_rate_out > 0) {
    ...
    // Set the miniscope ON pin
    digitalWrite(DIG_out_miniscope, HIGH);
