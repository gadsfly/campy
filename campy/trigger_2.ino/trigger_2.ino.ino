...
unsigned long SetFrameRate_(int num_pins) {
  // Wait to receive frame rate from Python or Serial Monitor...

  for ( int i = 0; i < num_pins; i++ ) {
    // Receive digital pin IDs
    while (Serial.available() == 0) {}
    int frame_rate_in = int( (unsigned int) Serial.parseFloat());
    Serial.print(input);
    if (i+1 < num_pins) { Serial.print(","); }
    ...
    // Append frame rate to array
    FRAME_RATE_OUT[i] = frame_rate_in;
    }
  }
  ...
  for ( int i = 0; i < num_pins; i++ ) {
  Serial.println("");
  Serial.print("Frame rate for pin ");
  Serial.print(DIG_out_pins[i]);
  Serial.print("set to: ");
  Serial.print(FRAME_RATE_OUT[i]);
  Serial.print(" fps.");
  }

  return FRAME_RATE_OUT[0];
}
